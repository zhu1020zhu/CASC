...
def init_args():
    ...
    data_args.add_argument("--bert_path", type=str, default="/home/zhubin/WorkSpace/Models/bert-base-uncased")
    ...
    if 'output_dir' not in train_params.keys() or train_params["output_dir"] is None:
        train_params[
            "output_dir"] = f'./checkpoints/{data_params["task"]}_{data_params["method"]}_{time.strftime("%m-%d-%H", time.localtime())}'
    if 'gnn_path' not in data_params.keys() or data_params["gnn_path"] is None:
        data_params[
            "gnn_path"] = f'/home/zhubin/WorkSpace/Finllm_fin_3090/checkpoints/gnn/{data_params["task"].lower()}/best_gnn_model.pt'

    if data_params["method"] == "noicl":
        data_params["is_ICL"] = "noicl"
    elif data_params["method"] == "fixicl":
        data_params["is_ICL"] = "fixicl"
    else:
        data_params["is_ICL"] = "icl"
...
def main():
    ...
    test_data = data_process(top_k=data_params["top_k"], task=data_params['task'], method=data_params['method'],
                             ICL=data_params['is_ICL'], paths=paths)
    ...
    print("========================================\n")
    print("第一轮验证：\n")
    evaluate(train_params['model_name_or_path'], test_data, task=data_params['task'],
             checkpoint_dir=train_params["output_dir"],
    ...
    #
    # print("第二轮验证：\n")
    # check_dir = '/root/autodl-tmp/Finllm_fin/checkpoints/FinEntity_correct_fixicl_07-10-10/checkpoint-100'
    # evaluate(train_params['model_name_or_path'], test_data, task=data_params['task'], checkpoint_dir=check_dir,
    #          temperature=0.1, top_p=0.9, finetuning_type=train_params["finetuning_type"])
    # torch.cuda.empty_cache()
    # print("第三轮验证：\n")
    # check_dir = '/root/autodl-tmp/Finllm_fin/checkpoints/SEntFiN_correct_fixicl_07-09-19/checkpoint-400'
    # evaluate(train_params['model_name_or_path'], test_data, task=data_params['task'], checkpoint_dir=check_dir,
    #          temperature=0.1, top_p=0.9, finetuning_type=train_params["finetuning_type"])
    # torch.cuda.empty_cache()
    # print("第四轮验证：\n")
    # check_dir = '/root/autodl-tmp/Finllm_fin/checkpoints/SEntFiN_fixicl_05-29-12/checkpoint-300'
    # evaluate(train_params['model_name_or_path'], test_data, task=data_params['task'], checkpoint_dir=check_dir,
    #          temperature=0.1, top_p=0.9, finetuning_type=train_params["finetuning_type"])
    # torch.cuda.empty_cache()
    # print("第五轮验证：\n")
    # check_dir = '//root/autodl-tmp/Finllm_fin/checkpoints/SEntFiN_fixicl_05-29-12/checkpoint-200'
    # evaluate(train_params['model_name_or_path'], test_data, task=data_params['task'], checkpoint_dir=check_dir,
    #          temperature=0.1, top_p=0.9, finetuning_type=train_params["finetuning_type"])
    # torch.cuda.empty_cache()
