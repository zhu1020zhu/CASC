    ...
    # 三元组和四元组抽取任务 添加对于 implicit aspect 和 Opinion 的考虑        
        ...
        for i, output in enumerate(outputs):
            for o in output:
                ...
                if sentiment == 'positive' or sentiment == 'POS':
                    ...
                    raise ValueError(f"Invalid sentiment: {sentiment}")
    elif task == 'FinEntity' or task == 'SEntFiN':
        sentiment_vectors = [[0, 0, 0] for _ in range(len(inputs))]
        # print(outputs[0], '\n')
        # print(outputs[0][0], '\n')
        # print(outputs[0][0], '\n')
        for i, output in enumerate(outputs):
            for o in output:
                sentiment = o[1]
                # print(sentiment)
                if sentiment == 'positive' or sentiment == 'POS' or sentiment == 'Positive':
                    sentiment_vectors[i][1] += 1
                elif sentiment == 'negative' or sentiment == 'NEG' or sentiment == 'Negative':
                    sentiment_vectors[i][0] += 1
                elif sentiment == 'neutral' or sentiment == 'NEU' or sentiment == 'Neutral':
                    sentiment_vectors[i][2] += 1
                else:
                    raise ValueError(f"Invalid sentiment: {sentiment}")
    else:
...
def compute_structural_similarity(outputs, task, threshold=0.3):
    ...
    similarity_matrix = []
    if task == 'ASPE' or task == 'MEMD_AS' or task == 'FinEntity' or task == 'SEntFiN':
