...
class SentenceAnalyzer:
                ...
                for word in words_group:
                    ...
                    if len(word.split(' ')) > 1:
                        ...
                        try:
                            ...
                            print(f"Warning: '{word}' not found in sentence '{sentence.split()}'")
    ...
    def linguistic_feature(self, sentences, outputs):
        ...
        elif self.task == 'FinEntity':
            center_words = [[[o[0]] for o in os] for os in outputs]
            # print("打乱顺序后第一个句子的中心词：", center_words[0])
            # print("第一个句子:", sentences[0])
        elif self.task == 'SEntFiN':
            center_words = [[[o[0]] for o in os] for os in outputs]
        elif self.task == 'EFSA':
            center_words = [[[o[0]] for o in os] for os in outputs]
        else:
        ...
        # 打印相似度，统计后再确定阈值
        print("\n语言学相似度均值：", similarity.mean(), "语言学相似度最大值：", similarity.max())   # finentity数据集 0.37006433748374445 0.45512110762641994
...
if __name__ == "__main__":
    ...
    # analyzer = SentenceAnalyzer(task='ASPE')
    analyzer = SentenceAnalyzer(task='FinEntity')
