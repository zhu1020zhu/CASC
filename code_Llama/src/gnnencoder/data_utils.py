...
def parse_output(s):
    ...
    elements = [re.split(r'##', group) for group in groups]
    # elements = [re.split(r', ', group) for group in groups]
    return elements
...
class ABSAGNNDataset(Dataset):
    def __init__(self, file_path, args):
        ...
        # 节点编码器
        self.tokenizer = BertTokenizer.from_pretrained('/home/zhubin/WorkSpace/Models/bert-base-uncased')
        self.model = BertModel.from_pretrained('/home/zhubin/WorkSpace/Models/bert-base-uncased').to(self.device)
        # 如果有多于一块的 GPU，并行编码
    ...
    def __getitem__(self, idx):
        inputs = self.data[idx]['gnn_input']
        outputs = parse_output(self.data[idx]['gnn_output'])
    ...
    def batch_generate_edge_features(self, inputs, outputs):
        ...
        edge_features = np.stack((lig_features, sen_features), axis=-1)
