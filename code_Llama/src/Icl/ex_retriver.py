...
class Ex_Retriver():
    def __init__(self, ex_file, paths=None, encode_method='sbert'):
        ...
        with open(ex_file, 'r') as f:
            ...
            self.pred_labels = []
            for d in data:
                self.sents.append(d['content'])
                self.labels.append(d['output'])
                self.pred_labels.append(d['pre'])
        self.data_dict = {}
        for sent, label, pred_label in zip(self.sents, self.labels, self.pred_labels):
            self.data_dict[sent] = [label, pred_label]
        ...
        if encode_method == 'sbert':
            ...
            # bert_model_path = '/home/zhubin/WorkSpace/Models/bert-base-uncased'
    ...
    def search_examples(self, query, top_k, verbose=False):
